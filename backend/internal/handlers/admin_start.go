...
type ElectionControlPayload struct {
	ElectionID int64 `json:"electionId"`
}
...
func StartElection(c *gin.Context) {
	...
	if privateKeyHex == "" || privateKeyHex == `${RELAYER_PRIVATE_KEY}` {
		IsVotingActive = true
		c.JSON(http.StatusOK, gin.H{"message": "Mode simulasi AKTIF"})
		return
	}
	...
	tx, err := evotingContract.StartElection(auth, electionIdObj)
	if err != nil {
	...
	if receipt.Status == 0 {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Transaksi StartElection ditolak (revert) oleh Smart Contract"})
		return
	}
